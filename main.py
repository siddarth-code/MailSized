...
PRICING_TIERS = {
    "gmail": {"base": 199, "tiers": {"under_3min": 199, "under_10min": 299, "under_20min": 449}},
    "outlook": {"base": 219, "tiers": {"under_3min": 219, "under_10min": 329, "under_20min": 499}},
    "other": {"base": 249, "tiers": {"under_3min": 249, "under_10min": 399, "under_20min": 549}},
}

UPSELLS = {"priority": 75, "transcript": 150}
...
async def process_video(
    background_tasks: BackgroundTasks, file: UploadFile = File(...), payment_intent: str = Form(...)
):
    ...
    # Add background task
    background_tasks.add_task(process_video_background, file_path, output_path)

    return JSONResponse(
        {"status": "processing", "file_id": file_id, "download_url": f"/download/{file_id}"}
    )
...
async def download_file(file_id: str):
    ...
    return FileResponse(
        file_path, filename=f"compressed_video_{file_id}.mp4", media_type="video/mp4"
    )
