...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # keep permissive for current flow
...
# NEW: robust stream probing (detect if audio/video streams exist)
...
def decide_two_pass(duration_sec: float, video_bps: int) -> bool:
    if duration_sec >= 120: return True
    if video_bps <= 600_000: return True
    return False
...
def auto_scale(width: int, height: int, video_bps: int) -> Tuple[int, int]:
    ...
    px = width * height
    if video_bps < 500_000: target_w, target_h = 854, 480
    elif video_bps < 900_000: target_w, target_h = 1280, 720
    else:
        if px > 1920 * 1080: target_w, target_h = 1920, 1080
    target_w -= target_w % 2; target_h -= target_h % 2
    ...
    if MAILGUN_KEY and MAILGUN_DOMAIN:
        try:
            r = requests.post(
                ...
                data={"from": f"MailSized Contact <{SENDER_EMAIL or 'no-reply@mailsized.com'}>","to":[OWNER_EMAIL],
                      "subject": f"[MailSized Contact] {subject}","text": f"From: {from_email}\n\n{body}"},
                timeout=10,
            )
...
def send_email_download(to_email: str, download_url: str) -> None:
    subject = "Your MailSized download"; body = f"Your file is ready: {download_url}\n"
    if MAILGUN_KEY and MAILGUN_DOMAIN:
        try:
            r = requests.post(
                f"https://api.mailgun.net/v3/{MAILGUN_DOMAIN}/messages",
                auth=("api", MAILGUN_KEY),
                data={"from": SENDER_EMAIL or "no-reply@mailsized.com","to":[to_email],"subject": subject,"text": body,
...
# ------------ Security headers (unchanged CSP from your last working rev) ------------
@app.middleware("http")
async def security_headers(request: Request, call_next):
    ...
    resp.headers["Content-Security-Policy"] = (
        ...
        "script-src 'self' https://cdnjs.cloudflare.com https://www.googletagmanager.com https://pagead2.googlesyndication.com; "
...
# ------------ Optional basic rate limit (unchanged) ------------
_RATE = {"tokens": {}, "capacity": 20, "refill": 20, "per": 60.0}
ADSENSE_CLIENT_ID = os.getenv("ADSENSE_CLIENT_ID", "ca-pub-7488512497606071")
ENABLE_ADSENSE = os.getenv("ENABLE_ADSENSE", "1") == "1"  # NEW: define so NameError cannot occur

def _adsense_context_for_request(request: Request) -> dict[str, str]:
    # Only emit tag when enabled AND client id present
    ...
    return {"adsense_tag": "", "adsense_client_id": ""}
...
# ------------ Views (unchanged except render helper) ------------
...
def blogs_index(request: Request):
...
async def upload(file: UploadFile = File(...), email: Optional[str] = Form(None)):
    ...
    temp_path = UPLOAD_DIR / f"{upload_id}.src"  # neutral extension
    ...
    try:
        ...
        has_video, has_audio = probe_stream_flags(str(temp_path))  # NEW
    ...
    meta = UploadMeta(
        ...
        has_video=has_video, has_audio=has_audio,  # NEW
    )
...
async def stripe_webhook(request: Request, stripe_signature: str = Header(None, alias="stripe-signature")):
    ...
    upload_id = metadata.get("upload_id"); job_id = metadata.get("job_id")
...
async def events(job_id: str):
    job = JOBS.get(job_id)
    if not job:
        dummy = JobState(job_id=job_id, upload=UploadMeta(upload_id="", src_path=Path(""), size_bytes=0, duration_sec=0, width=0, height=0), status=JobStatus.ERROR, message="Unknown job")
...
async def run_job(job: JobState):
    ...
    try:
        ...
        # NEW: safe stream mapping & audio handling
        stream_maps = ["-map", "0:v:0"]
        audio_args: list[str] = []
        if u.has_audio:
            stream_maps += ["-map", "0:a:0?"]  # optional audio
            audio_args = ["-c:a", "aac", "-b:a", str(a_bps)]
        else:
            audio_args = ["-an"]
        ...
        async def run_and_stream(cmd: list[str]) -> int:
            proc = await asyncio.create_subprocess_exec(
                *cmd, "-progress", "pipe:1", "-nostats", "-loglevel", "error",
                stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, preexec_fn=_preexec_ulimits
            )
            last_emit = 0.0
            while True:
            ...
            rc = await proc.wait()
            if rc != 0:
                # bubble up a friendlier error message if stderr has details
                err = (await proc.stderr.read()).decode("utf-8", "ignore").strip()
                raise RuntimeError(err or "FFmpeg failed")
            return rc

        passlog = str(OUTPUT_DIR / f"ffpass_{job.job_id}")  # NEW

        if do_two_pass:
            await run_and_stream(common + ["-b:v", str(v_bps), "-pass", "1", "-passlogfile", passlog, "-f", "mp4", "/dev/null"])
            await run_and_stream(common + ["-b:v", str(v_bps), "-pass", "2", "-passlogfile", passlog, str(out_path)])
        else:
            await run_and_stream(common + ["-b:v", str(v_bps), "-maxrate", str(int(v_bps * 1.2)), "-bufsize", str(int(v_bps * 2)), str(out_path)])
